...
cp ../target/debug/rust-git ad_test1/ 
 # 进入 ad_test1 目录
cd ad_test1
 # 执行 rust-git init
 ./rust-git init
...
echo "main分支修改内容" > test.txt
 # 添加并提交 test.txt
 ./rust-git add .
 commit_hash1=$(./rust-git commit -m "main" 2>&1)
 # 创建 temp1 和temp 2分支
...
echo "temp1分支修改内容" > test.txt
 # 添加并提交 test.txt
 ./rust-git add .
 commit_hash2=$(./rust-git commit -m "temp1" 2>&1)
 ./rust-git checkout main
 ./rust-git checkout temp2
 echo "temp2分支修改内容" > test.txt
  ./rust-git add .
 commit_hash3=$(./rust-git commit -m "temp2" 2>&1)
